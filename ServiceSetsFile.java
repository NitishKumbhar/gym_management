...
public class ServiceSetsFile {
     ...
     *
     * @param sets the path to the file on disk.
     * @return the ArrayList of strings representing body building Set in csv format.
     */
    private static ArrayList readFileToArrayList(Object sets) throws FileNotFoundException {
        ArrayList<String> csvLinesInArrayList = new ArrayList<>();

        Scanner scFile = new Scanner(new java.io.File((String) sets));
        for (; (scFile.hasNextLine());) {     // boucle tant qu'il y a une ligne suivante
